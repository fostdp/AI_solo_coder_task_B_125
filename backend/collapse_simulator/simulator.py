...
from .accelerator import AcceleratorInfo, get_xp, to_numpy, has_cupy
...
class CollapseSimulator:
    ...
    def __init__(self,
        ...
        self.g = 9.81
        self.use_gpu = bool(use_gpu) if use_gpu is not None else has_cupy()
    ...
    def generate_earthquake_motion(self, pga: float, duration: float,
        ...
        return generate_earthquake_motion(pga, duration, time_step, seed, self.use_gpu)
    ...
    def run_collapse_simulation(self, earthquake_pga: float, duration: float = 30.0,
        ...
        u_peak_np = to_numpy(u_peak).astype(float)
        floor_h_np = self.floor_heights
        env_np = to_numpy(envelope).astype(float)
        K_soft_factor = 1.0
        yield_drifts_np = to_numpy(yield_drifts).astype(float)
        ult_drifts_np = to_numpy(ult_drifts).astype(float)
        M_vec_np = to_numpy(M_vec).astype(float)
        forces_eq_np = to_numpy(forces_eq).astype(float)

        REPORT_EVERY = max(1, n_steps // 20)
        for step in range(n_steps):
            env = env_np[step]
            if step > 0:
                dmg_prev_np = to_numpy(damage_indices[step - 1]).astype(float)
            else:
            ...
            if step == 0:
                ...
                prev_disp_np = to_numpy(displacements[step - 1]).astype(float)
                prev_vel_np = to_numpy(velocities[step - 1]).astype(float)
        ...
        return {
            ...
            "accelerator": self._accelerator_info.device_name,
            "time_history": {
                "time_array": to_numpy(t).tolist(),
                "displacement_mm": (to_numpy(displacements) * 1000).tolist(),
                "drift_ratios": to_numpy(drift_ratios).tolist(),
                "damage_indices": to_numpy(damage_indices).tolist(),
                "base_shear": to_numpy(shear_forces).tolist()
            },
            "failure_sequence": failure_sequence,
            "failure_events": failure_sequence,
            "input_energy": {
                "peak_acceleration_g": earthquake_pga,
                "input_impulse": float(np.trapezoid(to_numpy(accel), to_numpy(t))),
                "arias_intensity": float(0.5 * np.pi * np.trapezoid(to_numpy(accel) ** 2, to_numpy(t)))
            }
        }
