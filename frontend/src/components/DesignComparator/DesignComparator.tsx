...
import {
  Row, Col, Card, Table, Tag, Typography, Space, Divider,
  Descriptions, Spin, Alert
} from 'antd';
import ReactECharts from 'echarts-for-react';
import api from '../../services/api';
import PagodaMiniModel, { PagodaInfo } from './PagodaMiniModel';

const { Title, Paragraph, Text } = Typography;

export interface DesignComparatorProps {
  defaultPagodaA?: string;
  defaultPagodaB?: string;
}

const DesignComparator: React.FC<DesignComparatorProps> = ({
  defaultPagodaA = 'yingxian',
  defaultPagodaB = 'gojunoto',
}) => {
    ...
    const load = async () => {
      ...
      try {
        const [a, b, r] = await Promise.all([
          api.get(`/new/dynasty/pagoda/${defaultPagodaA}`),
          api.get(`/new/dynasty/pagoda/${defaultPagodaB}`),
          api.get('/new/dynasty/compare', { params: { a: defaultPagodaA, b: defaultPagodaB } })
        ]);
        setPagodaA(a.data);
        setPagodaB(b.data);
