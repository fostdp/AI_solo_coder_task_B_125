...
import {
  ...
  Tabs, Divider, Progress, Empty, Spin, Statistic, Alert, Tooltip, Switch,
  Result, Badge, Steps
} from 'antd';
...
const FLOOR_COLORS = ['#ff4d4f', '#fa8c16', '#fadb14', '#52c41a', '#1890ff'];
...
const TowerCollapseView: React.FC<CollapseViewProps> = ({
  ...
  const maxH = Math.max(...floorHeights);
  const scale = 280 / (Math.max(...floorHeights.reduce((a, b) => a + b, 0)) * 1.3);
  const cumH: number[] = [];
  let s = 0;
        ...
        <Col xs={24} lg={7}>
          <Card title="仿真参数" size="small">
            <Space direction="vertical" style={{ width: '100%' }}>
              ...
              <Row>
                <Col span={12}><Button type="primary" block icon={<span>▶</span>} onClick={runSim} loading={running}>单次倒塌模拟</Button></Col>
